...
collection_range.onchange = function(){
    CreateRandomCollection()
    caption = document.getElementById("range-caption")
    caption.innerHTML = "collectionrange = " + collection_range.value
}
...
async function CreateRandomCollection()
{
    ...
    let width = Math.floor(window_x / (parseInt(range)+parseInt(collection_range.max/15)) );
    let gap = Math.max(1, Math.floor((window_x-(width*range))/150));
    array_HTML.style.gap = gap+"px";
